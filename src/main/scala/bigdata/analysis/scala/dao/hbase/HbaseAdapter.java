...
public class HbaseAdapter {
    ...
    public int init() {
        try {
            ...
            int poolSize= ConfigurationManager.getInteger("hbase.connection.poolsize");
            int waittime= ConfigurationManager.getInteger("hbase.connection.waittime.millis");
            int healthCheckInterval = ConfigurationManager.getInteger("hbase.connection.health.check.interval.second");
            String healthCheckTestTableName = ConfigurationManager.getProperty("hbase.connection.health.check.tableName");
            String hbaseSiteConfFile = ConfigurationManager.getProperty("hbase.site.path");
    ...
    /*
       根据rowkey获取获取特定行(rowkey),特定列簇，特定列 的最新版本的数据
     */
   public Result getRowByRowKey(String tableName,String family,String qualifier,String rowkey){
       Result result=null;
       HbaseConnection conn  = null;
       try {
           TableName tabName = TableName.valueOf(tableName);
    ...
    public List<Result> scanRowByValue(String tableName, String filterFamily, String filterColumn,
                                        String filterValue) {
        List<Result> list = new ArrayList<Result>();
        ...
        try {
            ...
            if (!admin.isTableEnabled(tabName)) {
                ...
                SingleColumnValueFilter filter=new SingleColumnValueFilter(Bytes.toBytes(filterFamily), Bytes.toBytes(filterColumn),
                        CompareFilter.CompareOp.EQUAL, Bytes.toBytes(filterValue));
                filterList.addFilter(filter);
                Scan scan = new Scan();
                scan.setFilter(filterList);
             /*   scan.addColumn(Bytes.toBytes(filterFamily), Bytes.toBytes(column));
                scan.addColumn(Bytes.toBytes(filterFamily), Bytes.toBytes(filterColumn));*/
                long begin = System.currentTimeMillis();
                results = table.getScanner(scan);
                long end = System.currentTimeMillis();
                if (null != results) {
